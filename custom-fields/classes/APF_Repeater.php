...
class APF_Repeater {
	...
	function pre_setup_fields( $post_id = 0 ) {
		//
	}

	function setup_field( $field, $prefix, $rand, $values ) {
	...
	function setup_fields( $post_id, $counter = 0, $values = false, $force = false ) {
	...
	function get_row_content( $post_id, $counter = 0, $value = false, $force = false ) {
		...
		$content .= MetaBox::get_fields_box( $this->fields );
		$content .= '</div>';
	...
	function get_content( $values, $post_id = 0, $submit = 'Добавить' ) {
